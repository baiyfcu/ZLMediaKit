...
SvacReader::SvacReader(const MediaTuple &tuple, const string &file_path,
...
SvacReader::SvacReader(const MediaTuple &tuple, const string &file_path, const ProtocolOption &option, toolkit::EventPoller::Ptr poller) {
    setup(tuple, file_path, option, std::move(poller));
}

void SvacReader::setup(const MediaTuple &tuple, const std::string &file_path, const ProtocolOption &option, toolkit::EventPoller::Ptr poller) {
    ...
    _demuxer = std::make_shared<SvacDemuxer>();
    ...
    if (tracks.empty()) {
        throw std::runtime_error(StrPrinter << "该svac3 data文件没有有效的track:" << _file_path);
    }
...
bool SvacReader::readNextSample() {
    bool keyFrame = false;
    bool eof = false;
    auto frame = _demuxer->readFrame(keyFrame, eof);
    if (!frame) {
...
void SvacReader::stopReadMP4() {
    _timer = nullptr;
}

void SvacReader::startReadMP4(uint64_t sample_ms, bool ref_self, bool file_repeat) {
    ...
    if (ref_self) {
        ...
        weak_ptr<SvacReader> weak_self = strong_self;
...
const SvacDemuxer::Ptr &SvacReader::getDemuxer() const {
    return _demuxer;
}

uint32_t SvacReader::getCurrentStamp() {
    return (uint32_t) (_seek_to + !_paused * _speed * _seek_ticker.elapsedTime());
}

void SvacReader::setCurrentStamp(uint32_t new_stamp) {
...
bool SvacReader::seekTo(MediaSource &sender, uint32_t stamp) {
...
bool SvacReader::pause(MediaSource &sender, bool pause) {
    if (_paused == pause) {
        return true;
    }
...
bool SvacReader::speed(MediaSource &sender, float speed) {
...
bool SvacReader::seekTo(uint32_t stamp_seek) {
...
bool SvacReader::close(MediaSource &sender) {
    _timer = nullptr;
    WarnL << "close media: " << sender.getUrl();
    return true;
}

MediaOriginType SvacReader::getOriginType(MediaSource &sender) const {
    return MediaOriginType::mp4_vod;
}

string SvacReader::getOriginUrl(MediaSource &sender) const {
    return _file_path;
}

toolkit::EventPoller::Ptr SvacReader::getOwnerPoller(MediaSource &sender) {
    return _poller;
}
