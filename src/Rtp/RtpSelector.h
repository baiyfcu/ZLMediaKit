...
public:
    ...
    RtpProcessHelper(const std::string &app, const std::string &stream_id, const std::weak_ptr<RtpSelector> &parent);
    ~RtpProcessHelper();
    void attachEvent();
...
public:
     ...
     * @return rtp处理器
     */
    RtpProcess::Ptr getProcess(const std::string &app, const std::string &stream_id, bool makeNew);
