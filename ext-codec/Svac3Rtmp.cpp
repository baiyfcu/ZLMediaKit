...
void Svac3RtmpDecoder::inputRtmp(const RtmpPacket::Ptr &pkt) {
...
void Svac3RtmpDecoder::splitFrame(const uint8_t *data, size_t size, uint32_t dts, uint32_t pts) {
    auto end = data + size;
    while (data + 4 < end) {
...
void Svac3RtmpDecoder::outputFrame(const char *data, size_t len, uint32_t dts, uint32_t pts) {
...
void Svac3RtmpEncoder::flush() {
    inputFrame(nullptr);
}

bool Svac3RtmpEncoder::inputFrame(const Frame::Ptr &frame) {
    if (!_rtmp_packet) {
    ...
    return _merger.inputFrame(frame, [this](uint64_t dts, uint64_t pts, const Buffer::Ptr &, bool have_key_frame) {
            // flags
            _rtmp_packet->buffer[0] = (uint8_t)RtmpVideoCodec::svac3 | ((uint8_t)(have_key_frame ? RtmpFrameType::key_frame : RtmpFrameType::inter_frame) << 4);
...
void Svac3RtmpEncoder::makeConfigPacket() {
    auto flags = (uint8_t)RtmpVideoCodec::svac3;
