...
static bool getAVCInfoSvac3(const char *sps, size_t sps_len, int &iVideoWidth, int &iVideoHeight, float &iVideoFps) {
    if (sps_len < 4) {
        return false;
    }
...
bool getAVCInfoSvac3(const string &strSps, int &iVideoWidth, int &iVideoHeight, float &iVideoFps) {
    return getAVCInfoSvac3(strSps.data(), strSps.size(), iVideoWidth, iVideoHeight, iVideoFps);
}
...
Svac3Track::Svac3Track(const string &sps, const string &pps, int sps_prefix_len, int pps_prefix_len) {
    _sps = sps.substr(sps_prefix_len);
    _pps = pps.substr(pps_prefix_len);
    Svac3Track::update();
}

CodecId Svac3Track::getCodecId() const {
    return CodecSVAC3;
}

int Svac3Track::getVideoHeight() const {
    return _height;
}

int Svac3Track::getVideoWidth() const {
    return _width;
}

float Svac3Track::getVideoFps() const {
    return _fps;
}

bool Svac3Track::ready() const {
    return true;//!_sps.empty() && !_pps.empty();
}

bool Svac3Track::inputFrame(const Frame::Ptr &frame) {
    using Svac3FrameInternal = FrameInternal<Svac3FrameNoCacheAble>;
    int type = SVAC3_TYPE(frame->data()[frame->prefixSize()]);

    return inputFrame_l(frame);

    if (( type == Svac3Frame::NAL_IDR) && ready()) {
        return inputFrame_l(frame);
    }
    ...
    bool ret = false;
    splitSvac3(frame->data(), frame->size(), frame->prefixSize(), [&](const char *ptr, size_t len, size_t prefix) {
        Svac3FrameInternal::Ptr sub_frame = std::make_shared<Svac3FrameInternal>(frame, (char *)ptr, len, prefix);
...
toolkit::Buffer::Ptr Svac3Track::getExtraData() const {
    CHECK(ready());
...
void Svac3Track::setExtraData(const uint8_t *data, size_t bytes) {
#ifdef ENABLE_MP4
    struct mpeg4_avc_t avc;
    memset(&avc, 0, sizeof(avc));
    if (mpeg4_avc_decoder_configuration_record_load(data, bytes, &avc) > 0) {
        ...
        if (size > 4) {
            splitSvac3((char *)config.data(), size, 4, [&](const char *ptr, size_t len, size_t prefix) {
                inputFrame_l(std::make_shared<Svac3FrameNoCacheAble>((char *)ptr, len, 0, 0, prefix));
            });
            update();
        }
    }
...
bool Svac3Track::update() {
    _width = 2560;
    _height = 1440;
    _fps = 25;
    return true;
    return getAVCInfoSvac3(_sps, _width, _height, _fps);
}

std::vector<Frame::Ptr> Svac3Track::getConfigFrames() const {
    if (!ready()) {
        return {};
    }
    return { createConfigFrame<Svac3Frame>(_sps, 0, getIndex()),
             createConfigFrame<Svac3Frame>(_pps, 0, getIndex()) };
}

Track::Ptr Svac3Track::clone() const {
    return std::make_shared<Svac3Track>(*this);
}

bool Svac3Track::inputFrame_l(const Frame::Ptr &frame) {
    bool ret = true;
    ...
    int type = SVAC3_TYPE(frame->data()[frame->prefixSize()]);
    switch (type) {
        case Svac3Frame::NAL_SPS: {
            _sps = string(frame->data() + frame->prefixSize(), frame->size() - frame->prefixSize());
            _latest_is_config_frame = true;
            ret = VideoTrack::inputFrame(frame);
            break;
        }
        case Svac3Frame::NAL_PPS: {
...
void Svac3Track::insertConfigFrame(const Frame::Ptr &frame) {
    if (!_sps.empty()) {
        VideoTrack::inputFrame(createConfigFrame<Svac3Frame>(_sps, frame->dts(), frame->getIndex()));
    }

    if (!_pps.empty()) {
        VideoTrack::inputFrame(createConfigFrame<Svac3Frame>(_pps, frame->dts(), frame->getIndex()));
    }
}

class Svac3Sdp : public Sdp {
public:
    Svac3Sdp(const string &strSPS, const string &strPPS, int payload_type, int bitrate) : Sdp(90000, payload_type) {
        _printer << "m=video 0 RTP/AVP " << payload_type << "\r\n";
        if (bitrate) {
            _printer << "b=AS:" << bitrate << "\r\n";
        }
        _printer << "a=rtpmap:" << payload_type << " " << getCodecName(CodecSVAC3) << "/" << 90000 << "\r\n";
...
Sdp::Ptr Svac3Track::getSdp(uint8_t payload_type) const {
    if (!ready()) {
        WarnL << getCodecName() << " Track未准备好";
        return nullptr;
    }
    return std::make_shared<Svac3Sdp>(_sps, _pps, payload_type, getBitRate() / 1024);
}
...
Track::Ptr getTrackByCodecId(int sample_rate, int channels, int sample_bit) {
    return std::make_shared<Svac3Track>();
}
...
Track::Ptr getTrackBySdp(const SdpTrack::Ptr &track) {
    ...
    return std::make_shared<Svac3Track>(sps, pps, 0, 0);
}

RtpCodec::Ptr getRtpEncoderByCodecId(uint8_t pt) {
    return std::make_shared<Svac3RtpEncoder>();
}

RtpCodec::Ptr getRtpDecoderByCodecId() {
    return std::make_shared<Svac3RtpDecoder>();
}

RtmpCodec::Ptr getRtmpEncoderByTrack(const Track::Ptr &track) {
    return std::make_shared<Svac3RtmpEncoder>(track);
}

RtmpCodec::Ptr getRtmpDecoderByTrack(const Track::Ptr &track) {
    return std::make_shared<Svac3RtmpDecoder>(track);
}

Frame::Ptr getFrameFromPtr(const char *data, size_t bytes, uint64_t dts, uint64_t pts) {
    return std::make_shared<Svac3FrameNoCacheAble>((char *)data, bytes, dts, pts, prefixSizeSvac3(data, bytes));
}

} // namespace

CodecPlugin svac3_plugin = { getCodec,
                            getTrackByCodecId,
                            getTrackBySdp,
